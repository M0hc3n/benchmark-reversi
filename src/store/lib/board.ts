...
export function checkFlipChess({
  board,
  player,
  row,
  col,
...
export function placeAndFlip({
  ...
  col,
  player,
}: FlipAllChess): ReadonlyDeep<Board> {
...
export function clearBoardCandidate(
  board: ReadonlyDeep<Board>
): ReadonlyDeep<Board> {
  return produce(board, (draft) => {
    for (let r = 0; r < 8; r += 1) {
      ...
      for (let c = 0; c < 8; c += 1) {
        ...
        if (!ch) {
          for (let i = 0; i < 8; i += 1) {
            const [rd, cd] = directions[i];
            if (
              checkFlipChess({ board: draft, player, row: r, col: c, rd, cd })
            ) {
              draft[r][c] = chess;
              count += 1;
              break;
...
export function countPlayerChess(
  board: ReadonlyDeep<Board>,
  player: string
): number {
  let count = 0;
