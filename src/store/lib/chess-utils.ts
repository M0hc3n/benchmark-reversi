...
export function countAroundChess(
  board: ReadonlyDeep<Board>,
  row: number,
  col: number
): number {
...
export function getBestPoint(
  scores: Array.NonEmptyArray<PointScore>,
  shouldRandom: boolean = true
): PointScore {
  const { score } = pipe(
    scores,
    Array.max(Order.struct({ score: Order.number }))
  );
